...
async function getOverview() {
  ...
  // Lazily generate the home-card blurb for the latest brain entry.
  // First render of a new entry costs ~$0.001 on Haiku 4.5; subsequent
  // renders read from BrainEntry.homeBlurb. If generation fails the
  // home card falls back to a body excerpt — never blank.
  const { ensureBrainBlurb, bodyExcerpt } = await import('@/lib/brain/blurb');
  const brainBlurb = brainLatest
    ? (await ensureBrainBlurb(brainLatest)) ?? bodyExcerpt(brainLatest.body)
    : null;
  ...
  return (
    <div className="flex flex-col gap-4 p-4">
      ...
      <section className="card">
        ...
        {brainLatest ? (
          <>
            {/* Plain-English one-liner generated by Haiku 4.5 on first
                render, cached on the row. This is the at-a-glance
                payload for the Auto-mode user; the technical title +
                body live on /brain for the deeper read. */}
            <p className="mt-2 text-sm leading-snug text-ink-100">
              {brainBlurb ?? brainLatest.body}
            </p>
            <div className="mt-2 flex items-center gap-2 text-[11px] text-ink-500">
              <BrainKindBadge kind={brainLatest.kind} />
              <span className="line-clamp-1">{brainLatest.title}</span>
            </div>
          </>
        ) : (

...
import { fetchStockHoldings } from '@/lib/holdings';
...
export default async function PositionsPage() {
  const user = await requirePageUser('/positions');
  const holdings = await fetchStockHoldings();

  // Roll-up line at the top: total market value + total P/L.
  ...
  const totalPlPct =
    totalCostBasis > 0n
      ? (Number(totalPl) / Number(totalCostBasis)) * 100
      : 0;
  ...
  return (
    <div className="flex flex-col gap-4 p-4">
      <header className="flex items-start justify-between pt-2">
        <div>
          <h1 className="text-2xl font-semibold">Stocks & ETFs</h1>
          <p className="mt-1 text-xs text-ink-400">
            {holdings.length} {holdings.length === 1 ? 'holding' : 'holdings'} · owned by {user.email}
          </p>
        </div>
      ...
      <section className="card">
        <HoldingsList
          holdings={view}
          emptyMessage="No stock positions yet. The agent will open some when the watchlist clears MOS."
        />
      </section>
    </div>
  );
